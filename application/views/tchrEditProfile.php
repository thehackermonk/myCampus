...
<body>
	...
	<nav class="navbar navbar-dark blue darken-4 fixed-top">
		<a class="navbar-brand" href="<?php echo base_url(); ?>index.php/tchrhome"><span class="pre-brandName">my</span>Campus</a>
		...
		<div class="row">
			...
			<div class="col-6">
				<div class="card card-body">
					<br>
					<form action="<?php echo base_url(); ?>index.php/tchrEditProfile/editProfile" method="post">
						...
						<div class="md-form">
							<input type="text" name="facname" id="facname" class="form-control" value="<?php echo $name; ?>" required>
							<label for="facname">Name</label>
						</div>
						<label>Gender : </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <div class="form-check form-check-inline">
                            <label class="form-check-label">
                            	<input type="radio" class="form-check-input" name="facgen" value="female" <?php echo ($gender=='female')?'checked':''?> required>
                                <i class="fa fa-venus genicon genicon-female" data-toggle="tooltip" title="Female"></i>
                            </label>
                        </div>
                        <div class="form-check form-check-inline">
                        	<label class="form-check-label">
                        		<input type="radio" class="form-check-input" name="facgen" value="male" <?php echo ($gender=='male')?'checked':''?> required>
                        		<i class="fa fa-mars genicon genicon-male" data-toggle="tooltip" title="Male"></i>
                        	</label>
                        </div>
                        <br><br>
                        <div class="md-form">
                            <textarea name="facadrs" class="adrs_box" placeholder="Address" required><?php echo $address; ?></textarea>
                        </div>
                        <div class="md-form">
                            <input type="email" id="fac_mail" name="fac_mail" class="form-control" value="<?php echo $mail; ?>" required>
                            <label for="fac_mail">E-mail</label>
                        </div>
                        <div class="md-form">
                            <input type="text" id="fac_cont" name="fac_cont" class="form-control" value="<?php echo $contact; ?>" required>
                            <label for="fac_cont">Contact</label>
                        </div>
                        <div class="md-form">
                        	<input type="text" id="fac_quali" name="fac_quali" class="form-control" value="<?php echo $qualification; ?>" required>
                        	<label for="fac_quali">Qualification</label>
                        </div>
                        <div class="input-group">
                        	...
                        	<select class="form-control" name="tchrSub" required>
                        		<?php
                        			...
                        			while($i<$totSub)
                        			{
                        				if($subject[$i]["id"]==$tchrsubid)
                        				{
                        					echo "<option value='".$subject[$i]['id']."' selected='selected'>".$subject[$i]['name']."</option>";
                        				}
                        				else
                        				{
